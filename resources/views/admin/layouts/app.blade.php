...
<div class="min-h-screen flex">
    ...
    <aside class="fixed inset-y-0 left-0 z-30 w-64 bg-slate-900 flex flex-col transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:z-auto"
        ...
        <nav class="flex-1 px-4 py-5 space-y-1 overflow-y-auto">
            @php
                $b = 'flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200';
                $i = 'text-slate-300 hover:text-white hover:bg-white/10';
                $a = 'text-orange-400 bg-orange-500/20 border border-orange-500/20 font-semibold';
            @endphp

            <p class="text-slate-500 text-xs font-semibold uppercase tracking-widest px-4 mb-2">General</p>

            <a href="{{ route('admin.dashboard') }}"
               class="{{ $b }} {{ request()->routeIs('admin.dashboard') ? $a : $i }}">
            ...
            <a href="{{ route('admin.users.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.users.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.commerces.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.commerces.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.categories.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.categories.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.reports.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.reports.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-slate-500 text-xs font-semibold uppercase tracking-widest px-4 pt-5 mb-2">Gestión</p>

            <a href="{{ route('admin.delivery.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.delivery.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.coupons.index') }}"
               class="{{ $b }} {{ request()->routeIs('admin.coupons.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-slate-500 text-xs font-semibold uppercase tracking-widest px-4 pt-5 mb-2">Configuración</p>

            <a href="{{ route('admin.admin-users.create') }}"
               class="{{ $b }} {{ request()->routeIs('admin.admin-users.*') ? $a : $i }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
